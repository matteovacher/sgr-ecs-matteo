...
class PhenotypeBuilder :
    ...
    def _query_other_cppn_weight_2_outputs(self, cppn, network_manager, coordinate1, coordinate2, one_towards_two, max_weight, type_output, type_env) :
        ...
        distance = self._distance(coordinate1, coordinate2)
        bias = np.pi if type_env == 0 else -np.pi
        if one_towards_two :
            inputs = [*coordinate1, *coordinate2, distance, bias]
        else :
            inputs = [*coordinate2, *coordinate1, distance, bias]
    ...
    def _connect_other_target_node_to_layer_without_bias_2_outputs(self, cppn, network_manager, target_node_coordinates, source_layer_coordinates, node_dict, one_towards_two, max_weight, max_bias, type_output, type_env) : 
        incomming_connections = []
        for source_node_coordinate in source_layer_coordinates:
            weight = self._query_other_cppn_weight_2_outputs(cppn, network_manager, source_node_coordinate, target_node_coordinates, one_towards_two, max_weight, type_output, type_env)
            incomming_connections.append((node_dict[tuple(source_node_coordinate)], weight))
        bias = 0
        return incomming_connections, bias
    
    def create_other_phenotype_network_without_bias_2_outputs(self, cppn, network_manager, all_layers_coordinates, activation_function, out_activation_function, agg, response, max_weight, max_bias, type_output, type_env) :
        ...
        for layer in all_layers_coordinates[1:] :
            ...
            for hidden_node in layer :
                one_towards_two = True 
                incomming_connections, bias = self._connect_other_target_node_to_layer_without_bias_2_outputs(cppn, network_manager, hidden_node, source_layer, node_dict, one_towards_two, max_weight, max_bias, type_output, type_env)
                act = out_activation_function if layer == all_layers_coordinates[-1] else activation_function
                node_evals.append(tuple( [node_dict[tuple(hidden_node)], act, agg, bias, response, incomming_connections] ))
            idx_current_source_layer += 1
