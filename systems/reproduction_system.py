        ...
        if self.type_genome == 'diploid' :
            ...
            while len(children_entity_ids) < max_population : 
                ...
                if len(parents_ids) >= 2 : 
                    ...
                    haploid_parent1 = registry.get_haploid(parent1)
                    haploid_parent2 = registry.get_haploid(parent2)
                    connections, biases, functions, dominances, nodes = self.genome_operator.cocrossover_with_modu_regu(haploid_parent1, haploid_parent2)
                    child_id = self.entity_manager.create_entity()
                    children_entity_ids.append(child_id)
                    registry.add_haploid_parents(child_id, parent1, parent2)
                    registry.add_haploid(child_id, connections, biases, functions, dominances, nodes)
...
class BothModularReproductionSystem : 

    def __str__(self) :
        return "Modular BothReproductionSystem, tournament is the selection process happening here with simple dominance for haploid "
        ...
        if self.type_genome == 'diploid' :
            ...
            while len(children_entity_ids) < max_population : 
                ...
                if len(parents_ids) >= 2 : 
                    ...
                    haploid_parent1 = registry.get_haploid(parent1)
                    haploid_parent2 = registry.get_haploid(parent2)
                    connections, biases, functions, dominances, nodes = self.genome_operator.crossover_with_modu_regu(haploid_parent1, haploid_parent2)
                    child_id = self.entity_manager.create_entity()
                    children_entity_ids.append(child_id)
                    registry.add_haploid_parents(child_id, parent1, parent2)
                    registry.add_haploid(child_id, connections, biases, functions, dominances, nodes)
...
class BothMeiosisModularReproductionSystem : 

    def __str__(self) :
        return "Modular BothReproductionSystem, tournament is the selection process happening here with simple dominance for haploid, i try to implement meiosis "
        ...
        if self.type_genome == 'diploid' :
            ...
            while len(children_entity_ids) < max_population : 
                ...
                if len(parents_ids) >= 2 : 
                    ...
                    genome_parent1 = registry.get_genome(parent1)
                    genome_parent2 = registry.get_genome(parent2)
                    connections, biases, functions, dominances, nodes, choice1, choice2  = self.genome_operator.meiosis_crossover(genome_parent1, genome_parent2)
                    child_id = self.entity_manager.create_entity()
                    children_entity_ids.append(child_id)
                    registry.add_parents(child_id, parent1, parent2, choice1, choice2)
                    registry.add_genome(child_id, connections[0], connections[1], biases[0], biases[1], functions[0], functions[1], dominances[0], dominances[1], nodes)
...
class BothUniformMeiosisModularReproductionSystem : 

    def __str__(self) :
        return "Modular BothReproductionSystem, tournament is the selection process happening here with simple dominance for haploid, i try to implement meiosis and uniform dominance"
    ...
    def process(self, registry) : 
        ...
        if self.type_genome == 'diploid' :
            ...
            for child_entity_id in children_entity_ids :
                if child_entity_id not in elitism :
                    genome = registry.get_genome(child_entity_id)
                    new_genome = self.genome_operator.mutate_uniform_with_modu_regu(genome, self.config.sigma_weight, self.config.sigma_bias, self.config.threshold_weight, self.config.threshold_bias, self.config.threshold_function, self.config.threshold_dominance, self.function_pool.pool)
                    registry.add_genome(child_entity_id, new_genome.connections[0], new_genome.connections[1], new_genome.biases[0], new_genome.biases[1], new_genome.functions[0], new_genome.functions[1], new_genome.dominances[0], new_genome.dominances[1], new_genome.nodes)                                        
            ...
            while len(children_entity_ids) < max_population : 
                ...
                if len(parents_ids) >= 2 : 
                    ...
                    haploid_parent1 = registry.get_haploid(parent1)
                    haploid_parent2 = registry.get_haploid(parent2)
                    connections, biases, functions, dominances, nodes = self.genome_operator.uniform_crossover_with_modu_regu(haploid_parent1, haploid_parent2)
                    child_id = self.entity_manager.create_entity()
                    children_entity_ids.append(child_id)
                    registry.add_haploid_parents(child_id, parent1, parent2)
                    registry.add_haploid(child_id, connections, biases, functions, dominances, nodes)
            ...
            for child_entity_id in children_entity_ids :
                if child_entity_id not in elitism :
                    haploid = registry.get_haploid(child_entity_id)
                    new_haploid = self.genome_operator.mutate_uniform_with_modu_regu(haploid, self.config.sigma_weight, self.config.sigma_bias, self.config.threshold_weight, self.config.threshold_bias, self.config.threshold_function, self.config.threshold_dominance, self.function_pool.pool)
                    registry.add_haploid(child_entity_id, new_haploid.connections, new_haploid.biases, new_haploid.functions, new_haploid.dominances, new_haploid.nodes)                                        
            ...
            self.results_manager.both_dominance_ties(self.genome_operator.dominance_ties, self.genome_operator.dominance_comparisons, self.generation, self.type_genome, self.type_env, False)
...
class BothSignalUniformMeiosisModularReproductionSystem : 
        ...
        if self.type_genome == 'diploid' :
            ...
            while len(children_entity_ids) < max_population : 
                ...
                if len(parents_ids) >= 2 : 
                    ...
                    genome_parent1 = registry.get_genome(parent1)
                    genome_parent2 = registry.get_genome(parent2)
                    connections, biases, functions, dominances, nodes, choice1, choice2  = self.genome_operator.meiosis_signal_crossover(genome_parent1, genome_parent2)
                    child_id = self.entity_manager.create_entity()
                    children_entity_ids.append(child_id)
                    registry.add_parents(child_id, parent1, parent2, choice1, choice2)
                    registry.add_genome(child_id, connections[0], connections[1], biases[0], biases[1], functions[0], functions[1], dominances[0], dominances[1], nodes)
