...
class BothNewModularCo2WiBiPhenotypeSystem :

    def __str__(self) :
        return "Modular BothPhenotypeSystem, build the phenotype from the genome with dominance, with 2 outputs, without biases and and with new genome in 3d not 5... with other logical rules "
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                ...
                cppn = registry.get_cppn(entity_id)
                body_substrate_shape = self.substrate_builder.extract_body_network_shape(self.config)
                body_substrate = self.substrate_builder.new_shape_into_coordinates(body_substrate_shape)
                act_func = np.tanh
                out_act_func = np.tanh
                node_evals, input_nodes, output_nodes = self.phenotype_builder.create_phenotype_network_without_bias_2_outputs(cppn, self.network_manager, body_substrate, act_func, out_act_func, sum, self.config.response, self.config.max_weight, self.config.max_bias, type_output = 0)
                ...
                grid_input_size = math.ceil(math.sqrt(observation_size))
                controller_substrate_shape = self.substrate_builder.extract_controller_network_shape(grid_input_size, self.config)
                controller_substrate = self.substrate_builder.new_shape_into_coordinates(controller_substrate_shape)
                node_evals, input_nodes, output_nodes = self.phenotype_builder.create_phenotype_network_without_bias_2_outputs(cppn, self.network_manager, controller_substrate, act_func, out_act_func, sum, self.config.response, self.config.max_weight, self.config.max_bias, type_output = 1)
                registry.add_controller_network(entity_id, node_evals, input_nodes, output_nodes)
...
class BothNewModular2WiBiPhenotypeSystem :

    def __str__(self) :
        return "Modular BothPhenotypeSystem, build the phenotype from the genome with simple dominance, with 2 outputs, without biases and and with new genome in 3d not 5... with other logical rules "
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                genome = registry.get_genome(entity_id)
                connections, bias, functions = self.genome_operator.dominance_with_modu_regu(genome)
                node_evals, input_nodes, output_nodes = self.network_manager.create_network_with_modu_regu(genome.nodes, connections, bias, functions, sum, response = 1)
                registry.add_cppn(entity_id, node_evals, input_nodes, output_nodes)
...
class BothEnvNewModular2WiBiPhenotypeSystem :
...
class BothModular2WiBiPhenotypeSystem :
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                ...
                cppn = registry.get_cppn(entity_id)
                body_substrate_shape = self.substrate_builder.extract_former_body_network_shape(self.config)
                body_substrate = self.substrate_builder.shape_into_coordinates(body_substrate_shape)
                act_func = np.tanh
                out_act_func = np.tanh
                node_evals, input_nodes, output_nodes = self.phenotype_builder.create_phenotype_network_without_bias_2_outputs(cppn, self.network_manager, body_substrate, act_func, out_act_func, sum, self.config.response, self.config.max_weight, self.config.max_bias, type_output = 0)
                ...
                body_network = registry.get_body_network(entity_id)
                robot_grid = self.robot_generator.generate_former_robot_body_from_network(body_network, self.network_manager, self.config.body_shape)
                if not self.robot_generator.is_valid_robot(robot_grid) :
                    fitness, finished  = -1000, True
                    registry.add_fitness(entity_id, fitness, finished)
                    continue 
...
class BothEnvModular2WiBiPhenotypeSystem :

    def __str__(self) :
        return "Modular BothPhenotypeSystem, build the phenotype from the genome with simple dominance, with 2 outputs, without biases and and with new genome in 5d not in 3  with other logical rules "
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                ...
                body_network = registry.get_body_network(entity_id)
                robot_grid = self.robot_generator.generate_former_robot_body_from_network_and_env(body_network, self.network_manager, self.config.body_shape, self.type_env)
                if not self.robot_generator.is_valid_robot(robot_grid) :
                    fitness, finished  = -1000, True
                    registry.add_fitness(entity_id, fitness, finished)
                    if registry.has_controller_network(entity_id) :
...
class BothOtherEnvModular2WiBiPhenotypeSystem :

    def __str__(self) :
        return "Modular BothPhenotypeSystem, build the phenotype from the genome with simple dominance, with 2 outputs, without biases and and with new genome in 5d not in 3  and more importantly with another input to cppn which is a bias depending on the env type"
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                ...
                act_func = np.tanh
                out_act_func = np.tanh
                node_evals, input_nodes, output_nodes = self.phenotype_builder.create_other_phenotype_network_without_bias_2_outputs(cppn, self.network_manager, body_substrate, act_func, out_act_func, sum, self.config.response, self.config.max_weight, self.config.max_bias, type_output = 0, type_env = self.type_env)

                registry.add_body_network(entity_id, node_evals, input_nodes, output_nodes)

                body_network = registry.get_body_network(entity_id)
                robot_grid = self.robot_generator.generate_other_robot_body_from_network_and_env(body_network, self.network_manager, self.config.body_shape)
                if not self.robot_generator.is_valid_robot(robot_grid) :
                    fitness, finished  = -1000, True
                    registry.add_fitness(entity_id, fitness, finished)
                    if registry.has_controller_network(entity_id) :
                ...
                controller_substrate_shape = self.substrate_builder.extract_former_controller_network_shape(grid_input_size, self.config)
                controller_substrate = self.substrate_builder.shape_into_coordinates(controller_substrate_shape)
                node_evals, input_nodes, output_nodes = self.phenotype_builder.create_other_phenotype_network_without_bias_2_outputs(cppn, self.network_manager, controller_substrate, act_func, out_act_func, sum, self.config.response, self.config.max_weight, self.config.max_bias, type_output = 1, type_env = self.type_env)
                registry.add_controller_network(entity_id, node_evals, input_nodes, output_nodes)
...
class BothUniformEnvModular2WiBiPhenotypeSystem :
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            self.genome_operator.reset_dominance_counters()
            ...
            self.results_manager.both_dominance_ties(self.genome_operator.dominance_ties, self.genome_operator.dominance_comparisons, current_gen, self.type_genome, self.type_env, self.switched)
...
class BothSignalUniformEnvModular2WiBiPhenotypeSystem :
    ...
    def process(self, registry) :
        ...
        if self.type_genome == 'diploid' :
            ...
            for entity_id in entity_ids : 
                genome = registry.get_genome(entity_id)
                connections, bias, functions = self.genome_operator.signal_uniform_dominance_with_modu_regu(genome, self.type_env)
                node_evals, input_nodes, output_nodes = self.network_manager.create_network_with_modu_regu(genome.nodes, connections, bias, functions, sum, response = 1)
                registry.add_cppn(entity_id, node_evals, input_nodes, output_nodes)
                ...
                body_network = registry.get_body_network(entity_id)
                robot_grid = self.robot_generator.generate_former_one_signal_robot_body_from_network_and_env(body_network, self.network_manager, self.config.body_shape, self.type_env)
                if not self.robot_generator.is_valid_robot(robot_grid) :
                    fitness, finished  = -1000, True
                    registry.add_fitness(entity_id, fitness, finished)
                    if registry.has_controller_network(entity_id) :
