...
class Both0initModularBuildSystem() :
    def __str__(self) :
        return "modular both build system" 
    ...
    def process(self, registry) :
        if self.type_genome == 'diploid' :
            ...
            for _ in range(self.config.population) :
                entity_id = self.entity_manager.create_entity()
                connections1, connections2, bias1, bias2, activation_functions1, activation_functions2, dominance1, dominance2, nodes_by_layer = self.genome_operator.generate_first_generation_of_genome_with_modu_regu_0_init(nodes_by_layer, self.function_pool.pool)
                registry.add_genome(entity_id, connections1, connections2, bias1, bias2, activation_functions1, activation_functions2, dominance1, dominance2, nodes_by_layer)
                registry.add_age(entity_id, 1)
            self.results_manager.starting_both(self.config, self.type_genome)
            ...
            for _ in range(self.config.population) :
                entity_id = self.entity_manager.create_entity()
                connections, biases, activation_functions, dominances, nodes_by_layer = self.genome_operator.generate_first_generation_of_genome_with_modu_regu_0_init(nodes_by_layer, self.function_pool.pool) 
                registry.add_haploid(entity_id, connections, biases, activation_functions, dominances, nodes_by_layer)
                registry.add_age(entity_id, 1)
            self.results_manager.starting_both(self.config, self.type_genome)
...
class BothUniformModularBuildSystem() :
    def __str__(self) :
        return "modular both build system" 
    ...
    def process(self, registry) :
        if self.type_genome == 'diploid' :
            ...
            for _ in range(self.config.population) :
                entity_id = self.entity_manager.create_entity()
                connections1, connections2, bias1, bias2, activation_functions1, activation_functions2, dominance1, dominance2, nodes_by_layer = self.genome_operator.generate_uniform_first_generation_of_genome_with_modu_regu(nodes_by_layer, self.function_pool.pool)
                registry.add_genome(entity_id, connections1, connections2, bias1, bias2, activation_functions1, activation_functions2, dominance1, dominance2, nodes_by_layer)
                registry.add_age(entity_id, 1)
            self.results_manager.starting_both(self.config, self.type_genome)
            ...
            for _ in range(self.config.population) :
                entity_id = self.entity_manager.create_entity()
                connections, biases, activation_functions, dominances, nodes_by_layer = self.genome_operator.generate_uniform_first_generation_of_genome_with_modu_regu(nodes_by_layer, self.function_pool.pool) 
                registry.add_haploid(entity_id, connections, biases, activation_functions, dominances, nodes_by_layer)
                registry.add_age(entity_id, 1)
            self.results_manager.starting_both(self.config, self.type_genome)
